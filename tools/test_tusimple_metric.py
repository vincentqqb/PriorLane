...
from mmseg.apis import multi_gpu_test, single_gpu_test, single_gpu_test_tusimple
...
def main():
    ...
    if not distributed:
        ...
        outputs = single_gpu_test_tusimple(model, data_loader, args.show, args.show_dir,
    ...
    if rank == 0:
        ...
        pred_dict_list = []  
        fo = open(os.path.join(dataset.data_root,"pred.json"),"w")
        for i in range(len(img_info)):
            if i % 100 ==0:
                print(i)
            res_dict = {"raw_file":img_info[i]["filename"], "lanes":outputs[i], "run_time":5}
            pred_dict_list.append(json.dumps(res_dict))
        fo.write("\n".join(pred_dict_list))
        fo.close()
